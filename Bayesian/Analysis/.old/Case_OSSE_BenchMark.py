...
#str_StartTime = "2019-01-01_00:00:00"
#str_EndTime = "2019-01-05_18:00:00"
...
case = Analyst(CaseName = CaseName)#, Start = StartTime, End = EndTime)
...
def draw_pptr(time):
    ...
    ax_prior, cs_prior = case.plot2D(prior, fig = bcp1.cfig, subplot = (3,2,1), vmin = vmin, vmax = vmax)
    ax_truth, cs_truth = case.plot2D(truth, fig = bcp1.cfig, subplot = (3,2,2), vmin = vmin, vmax = vmax)
    ax_posterior, cs_posterior = case.plot2D(posterior, fig = bcp1.cfig, subplot = (3,2,3), vmin = vmin, vmax = vmax)
    ax_RQ, cs_RQ = case.plot2D(RQ, fig = bcp1.cfig, subplot = (3,2,4), vmin = vmin, vmax = vmax)
    ax_pmp, cs_pmp = case.plot2D(posterior - prior, fig = bcp1.cfig, subplot = (3,2,5), vmin = vmin, vmax = vmax)
    ...
    draw_cbar(cs = cs_pmt, bcp = bcp1, loc = (0.15, 0.05, 0.7, 0.03), label = "umol/m^2/s", orientation = "horizontal")
...
def opti_prog(time):
    ...
    for i_reduced in range(N_reduced):
        ...
        ax_obs, cs_obs = case.draw_OBS_scatter(time = recepTime, fig = bcp2.cfig, subplot = (sub1, sub2, sub3), size = 50, draw_cbar = False)
